...
> [!WARNING]
> It uses a [Nerdfont](https://www.nerdfonts.com/font-downloads) by default for the icons, so it is recommended to have a Nerdfont set as your terminal font. Otherwise, you can set the icons to regular text with the options below.
...
```
- Most options are changeable on-the-fly. If they do not update with the tmux command `source-file ~/.tmux.conf` (or your conf location), you need to close the `tmux` instance and run it again (no sessions remaining).
